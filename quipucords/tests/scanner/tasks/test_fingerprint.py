...
from tests.factories import DetailsReportFactory, ScanJobFactory, ScanTaskFactory
...
def test_fingerprint_happy_path_details_report_already_exists(mocker):
    ...
    scan_job = ScanJobFactory(status=ScanTask.RUNNING, report_id=None)
    details_report = DetailsReportFactory(scanjob=None)
    ...
    assert scan_job.details_report == details_report
    assert scan_job.report_id == details_report.deployment_report.id
...
def test_fingerprint_happy_path_creates_details_report(mocker):
    ...
    scan_job = ScanJobFactory(
        status=ScanTask.RUNNING, details_report=None, report_id=None
    )
    scan_task = ScanTaskFactory(
...
def test_fingerprint_fails_when_create_details_report_fails(mocker):
    ...
    assert scan_job.details_report is None
