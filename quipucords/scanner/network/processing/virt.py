...
class ProcessVirtType(process.Processor):
    ...
    def process(output, dependencies):
        ...
        if dependencies is not None and bool(dependencies):
            if dependencies.get('internal_have_dmidecode') and \
                    dependencies.get('internal_sys_manufacturer'):
                manufacturer = dependencies.get(
                    'internal_sys_manufacturer').lower()
