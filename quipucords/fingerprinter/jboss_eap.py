...
def classify_jar_versions(jar_versions):
    ...
    for version_data in jar_versions:
        ...
        versions.add(EAP_CLASSIFICATIONS.get(version, unknown_release))
    return versions
...
def verify_classification(classification):
    ...
    if "wildfly" in classification.lower() or "jbossas" in classification.lower():
        return False
    return True
...
def is_eap_manifest_version(manifest_dict):
    ...
    for _, manifest in manifest_dict.items():
        if isinstance(manifest, str):
            ...
            logger.warning(
                "Expected a dictionary of strings for %s, "
                "but found %s.  Invalid value: %s",
                EAP_HOME_MODULES_VERSION,
                type(manifest),
                manifest,
            )
    return Product.ABSENT
...
FACTS = [
    ...
    {NAME: JBOSS_EAP_EAP_HOME, PRESENCE: any_value_true},
    {
    ...
    {NAME: EAP_HOME_README_TXT, PRESENCE: Product.ABSENT},
    {
...
def detect_jboss_eap(source, facts):
    ...
    if versions:
        versions = [
            ...
            if not version.lower().startswith("wildfly")
            and not version.lower().startswith("jbossas")
        ]
        if not versions:
