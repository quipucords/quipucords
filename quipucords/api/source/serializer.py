...
class SourceSerializerBase(ModelSerializer):
    ...
    def validate_ipaddr_list(hosts):
        ...
        for host in ipaddr_list:
            try:
                normalized_hosts.extend(
                    SourceSerializer.classify_and_validate_host(host)
                )
        ...
        if host_errors:
            error_message = ", ".join(
                error.detail.pop() for error in host_errors if error.detail
            )
    ...
    @staticmethod
    def classify_and_validate_host(host):
        """
        Classify and validate a host as an IP, CIDR, hostname, or Ansible range.

        Returns a list of valid hosts or raises a ValidationError.
        """
        if SourceSerializer.is_valid_ip(host):
            return [host]

        if SourceSerializer.is_valid_cidr(host):
            return SourceSerializer.expand_cidr(host)

        if SourceSerializer.is_valid_hostname(host):
            return [host]

        if SourceSerializer.is_valid_ansible_range(host):
            return SourceSerializer.normalize_ansible_range(host)

        raise ValidationError(_(messages.NET_INVALID_HOST % (host,)))
    ...
    def is_valid_hostname(host):
        ...
        return FQDN(host).is_valid

    @staticmethod
    def is_valid_ansible_range(host):
        """Check if the input is likely an Ansible range."""
        return bool(re.search(r"\[\d+:\d+\]", host))

    @staticmethod
    def normalize_ansible_range(host):
        """Normalize an Ansible range into a list of hosts."""
        try:
            return expand_hostname_range(host)
        except (ValueError, AnsibleError):
            raise ValidationError(_(messages.NET_INVALID_RANGE_FORMAT % (host,)))
