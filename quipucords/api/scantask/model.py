...
class ScanTask(models.Model):
    ...
    def get_facts(self):
        ...
        for inspect_result in (
            InspectResult.objects.filter(result_set__in=self.result_sets.all())
    ...
    def cleanup_facts(self, identity_key):
        ...
        valid_identity_raw_facts = (
            RawFact.objects.filter(
                inspect_result__result_set__in=self.result_sets.all(),
                name=identity_key,
            )
        ...
        InspectResult.objects.filter(result_set__in=self.result_sets.all()).exclude(
            facts__in=valid_identity_raw_facts
        ).delete()
    ...
    def get_result(self):
        ...
        if self.scan_type == ScanTask.SCAN_TYPE_INSPECT:
            return InspectResult.objects.filter(
                result_set__in=self.result_sets.all()
            ).all()
