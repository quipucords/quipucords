...
from .featureflag import FeatureFlag, get_feature_flags_from_env
...
def test_get_feature_flags_from_env(env_name, env_value, feature_name,
                                    feature_value):
...
def test_when_value_cant_be_cast_to_int():
    """Tests if function does not add or update key-values to default dict.

    if value can`t be cast to int.
    """
    with mock.patch.dict(os.environ, (
            {'QPC_FEATURE_OVERALL_STATUS': 'wrong'})):
        dict_with_test_flags = get_feature_flags_from_env()
        assert 'OVERALL_STATUS' in dict_with_test_flags
        assert dict_with_test_flags['OVERALL_STATUS'] is False
...
def test_when_int_is_not_valid_value_for_env(
        env_name, env_value, feature_name):
    """Tests if function does not add values that are not present in the.

    dict for valid env variables.
    """
    with mock.patch.dict(os.environ, ({env_name: env_value})):
        dict_with_test_flags = get_feature_flags_from_env()
...
def test_if_function_is_not_case_sensitive(env_name, env_value, feature_name,
                                           feature_value):
    """Tests if function is not case-sensitive and env names are stored following.
...
def test_is_feature_active(setup_feature_flag_instance_for_tests):
    ...
    assert setup_feature_flag_instance_for_tests.is_feature_active(
        'TEST') is True
    assert setup_feature_flag_instance_for_tests.is_feature_active(
        'OVERALL_STATUS') is False
    with pytest.raises(ValueError):
        assert setup_feature_flag_instance_for_tests.is_feature_active(
            'FALSE_ATTRIBUTE') is True
