...
1. Download the server container image by entering the following command::
    ...
    # curl -k -O -sSL https://github.com/quipucords/quipucords/releases/download/0.0.43/quipucords.0.0.43.tar.gz


2. Load the container image into the local Docker registry with the following command::

    # sudo docker load -i quipucords.0.0.43.tar.gz

The output appears similar to the following example::

    Loaded image: quipucords:0.0.43
...
The output appears similar to the following example::
  ...
  quipucords              0.0.43               fdadcc4b326f        3 days ago          969MB
