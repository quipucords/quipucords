...
**qpc cred add --name=** *name* **--type=** *(network | vcenter | satellite)* **--username=** *username* **(--password | --sshkeyfile=** *key_file* **)** **[--sshpassphrase]** **[--sudo-password]** **--become_method=** *(sudo | su | pbrun | pfexec | doas | dzdo | ksu | runas )*
...
``--become_method=become_method``
  Sets the method to become when running an ansible playbook. Must be ``sudo``, ``su``, ``pbrun``, ``pfexec``, ``doas``, ``dzdo``, ``ksu``, ``runas``. The default is set to 'sudo' when the credential type is network.


The information in a credential, such as a password, sudo password, SSH keys, the become_method, or even the username, might change. For example, network security might require passwords to be updated every few months. Use the ``qpc cred edit`` command to change the SSH credential information in a credential. The parameters for ``qpc cred edit`` are the same as those for ``qpc cred add``.

**qpc cred edit --name=** *name* **--username=** *username* **(--password | --sshkeyfile=** *key_file* **)** **[--sshpassphrase]** **[--sudo-password]** **--become_method=** *(sudo | su | pbrun | pfexec | doas | dzdo | ksu | runas )*
